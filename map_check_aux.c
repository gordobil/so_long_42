...
int	find_exit(int y, int x)
{
	if (g_map_dup[y][x] == 'E')
		return (0);
	else if (g_map_dup[y][x] != '1')
		g_map_dup[y][x] = 'P';
	if ((x - 1 >= 0) && (g_map_dup[y][x - 1] != '1') &&
		(g_map_dup[y][x - 1] != 'P'))
	{
		if (find_exit(y, x - 1) == 0)
			return (0);
	}
	else if ((x + 1 < g_width) && (g_map_dup[y][x + 1] != '1') &&
			(g_map_dup[y][x + 1] != 'P'))
	{
		if (find_exit(y, x + 1) == 0)
			return (0);
	}
	else if ((y - 1 >= 0) && (g_map_dup[y - 1][x] != '1') &&
			(g_map_dup[y - 1][x] != 'P'))
	{
		if (find_exit(y - 1, x) == 0)
			return (0);
	}
	else if ((y + 1 < g_height) && (g_map_dup[y + 1][x] != '1') &&
			(g_map_dup[y + 1][x] != 'P'))
	{
		if (find_exit(y + 1, x) == 0)
			return (0);
